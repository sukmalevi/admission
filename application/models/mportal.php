...
class Mportal extends CUTI_Model{
	...
	function mEmailKonfirm($kode, $bayar){
			...
			// Validasi email terlebih dahulu.
				// Email configuration
					...
					//$this->email->to("div.it@paramadina.ac.id");
	...
	function mPaidDaftarUlang($kode, $saudara, $jenjang = ''){
		$angsuran = '';
		...
		SUBSTR(metodBayarDaftarUlang, 12, 3) as lulusan, k.alumni_s1 as alumni
		FROM adis_smb_form f 
		LEFT JOIN adis_smb_usr u ON u.kode = f.kode
		LEFT JOIN adis_buka_smb b ON b.kode = f.bukaSmb
		LEFT JOIN adis_periode d ON d.kode = b.periode
		...
		switch($jenjang){
			case '1':
				IF  ($jalur_code == 'KP'){
						...
						$metodBayar = "SELECT p.*, p.kode as kode_pembayaran 
						...
						if ($pembayaran == 1){
							...
							$totalAll =  $biaya->angsuran1 + $biaya->angsuran2 + $biaya->angsuran3 + $biaya->angsuran4 + $biaya->angsuran5 ;
						}
